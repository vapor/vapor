...
final class HTTPHeaderTests: XCTestCase {
    ...
    func testComplexCookieParsing() throws {
        var headers = HTTPHeaders()
        do {
            headers.add(name: .setCookie, value: "simple-cookie=value; Path=/; SameSite=None")
            ...
            XCTAssertEqual(headers.setCookie?.all.count, 3)
          
            let simpleCookie = try XCTUnwrap(headers.setCookie?["simple-cookie"])
            XCTAssertFalse(simpleCookie.isSecure)
            XCTAssertFalse(simpleCookie.isHTTPOnly)
            XCTAssertFalse(simpleCookie.isPartitioned)
            ...
            XCTAssertTrue(siwaState.isPartitioned)
