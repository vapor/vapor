...
final class HTTPHeaderTests: XCTestCase {
    ...
    func testComplexCookieParsing() throws {
        ...
        do {
            headers.add(
                name: .setCookie, value: "SIWA_STATE=CJKxa71djx6CaZ0MwRjtvtJ5Zub+kfaoIEZGoY3wXKA=; Path=/; SameSite=None; HttpOnly; Secure")
            headers.add(
                name: .setCookie,
                value:
            ...
            XCTAssertEqual(vaporSession.expires, Date(timeIntervalSince1970: 1_622_645_877))
            XCTAssertTrue(siwaState.isHTTPOnly)
            XCTAssertTrue(siwaState.isSecure)
        }
    }
    ...
    func testLinkHeaderParsing() throws {
        ...
        let b = try XCTUnwrap(headers.links?.dropFirst(1).first)
        let c = try XCTUnwrap(headers.links?.dropFirst(2).first)
        let d = try XCTUnwrap(headers.links?.dropFirst(3).first)
