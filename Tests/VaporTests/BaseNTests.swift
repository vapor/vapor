...
struct BaseNTests {
    ...
    func check(with instance: BaseNEncoding, name: String, expected: [String], sourceLocation: SourceLocation = #_sourceLocation) {
        ...
        for (elem, expected) in zip(self.checks, expected) {
            ...
            if printVectors {
                print(
                    instance.bits > 6 ? "\(encodedBytes)" : "\"\(encodedString)\"", terminator: elem.count == Self.vector.count ? "" : ", ")
            } else {
                ...
                #expect(
                    utf8ReadyBytesWithoutNullTermination.flatMap { String(validating: $0, as: UTF8.self)?[...] } == elem,
                    "\(name) - \(elem) - \(decodedBytes ?? [])", sourceLocation: sourceLocation)
            }
        }
        if printVectors {
