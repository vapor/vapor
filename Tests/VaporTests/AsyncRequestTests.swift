...
final class AsyncRequestTests: XCTestCase {
    ...
    // https://github.com/vapor/vapor/issues/3202
    func testLargeBodyCollectionDoesntCrash() async throws {
        app.http.server.configuration.hostname = "127.0.0.1"
        app.http.server.configuration.port = 0

        app.on(.POST, "upload", body: .stream) { request -> String in
            let buffer = try await request.body.collect(upTo: Int.max)
            return "Received \(buffer.readableBytes) bytes"
        }

        app.on(.POST, "testasyncstream", body: .stream) { request in
            var totalBytes = 0
            for try await chunk in request.body {
                totalBytes += chunk.readableBytesView.count
            }
            print("Read \(totalBytes) bytes from request")
            return HTTPResponseStatus(statusCode: 202)
        }
        ...
        guard
            ...
            XCTFail(
                "couldn't get ip/port from \(app.http.server.shared.localAddress.debugDescription)")
            return
        }

        var bigBody = ByteBuffer(repeating: 0x41, count: 600 * 1024 * 1024)
        var request1 = HTTPClientRequest(url: "http://\(ip):\(port)/upload")
        request1.method = .POST
        request1.body = .bytes(bigBody)

        for _ in 0..<10 {
            let response: HTTPClientResponse = try await app.http.client.shared.execute(
                request1, timeout: .seconds(5)
            )
            XCTAssertEqual(response.status, .ok)
            let body = try await response.body.collect(upTo: 1024 * 1024)
        ...
        var request2 = HTTPClientRequest(url: "http://\(ip):\(port)/testasyncstream")
        request2.method = .POST
        request2.headers = ["Content-Type": "application/octet-stream"]
        let body = try XCTUnwrap(bigBody.readData(length: bigBody.readableBytes))
        request2.body = .stream(body.async, length: .known(bigBody.readableBytes))

        for _ in 0..<10 {
            let response: HTTPClientResponse = try await app.http.client.shared.execute(
                request2, timeout: .seconds(5)
            )
