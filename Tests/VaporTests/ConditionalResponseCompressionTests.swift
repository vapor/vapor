...
final class ConditionalResponseCompressionServerTests: XCTestCase, @unchecked Sendable {
    ...
    func testAutoDetectedType() async throws {
        ...
        try await assertUncompressed(app.http.server.configuration.responseCompression)
        /// Default case
    ...
    func testUnknownType() async throws {
        app.get("resource") { request in
            ...
            /// Not explicitely marked as compressible or not.
    ...
    func testImage() async throws {
        app.get("resource") { request in
            ...
            /// PNGs are explicitely called out as incompressible.
            return compressiblePayload.encodeResponse(status: .ok, headers: headers, for: request)
        }
    ...
    func testVideo() async throws {
        app.get("resource") { request in
            ...
            /// Videos are explicitely called out as incompressible, but as a class.
            return compressiblePayload.encodeResponse(status: .ok, headers: headers, for: request)
        }
    ...
    func testText() async throws {
        app.get("resource") { request in
            ...
            /// Text types are explicitely called out as compressible, but as a class.
            return compressiblePayload.encodeResponse(status: .ok, headers: headers, for: request)
        }
    ...
    func testEnabledByResponse() async throws {
        ...
        try await assertCompressed(app.http.server.configuration.responseCompression)
        /// Default case
        try await assertUncompressed(.forceDisabled)
    ...
    func testNoopsDisabledByRouteButReset() async throws {
        app.responseCompression(.unset).responseCompression(.disable).responseCompression(.unset).responseCompression(.useDefault)
            .responseCompression(.unset).get("resource") { request in
                var headers = HTTPHeaders()
                headers.contentType = unknownType
                return compressiblePayload.encodeResponse(status: .ok, headers: headers, for: request)
            }
...
final class ConditionalResponseCompressionRouteTests: XCTestCase, @unchecked Sendable {
    ...
    func testRoutingPrioritizesMultipleResponse() async throws {
        ...
        try await assertResponseCompression(
            middleware: disabledMiddleware, responder: multipleResponse, compressionValue: "enable, disable")
