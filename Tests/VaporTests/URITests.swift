...
func XCTAssertURIComponents(
    ...
    do {
        ...
        XCTAssertEqual(
            uri.host, host?.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed), "(host) \(message())", file: file, line: line)
        XCTAssertEqual(uri.port, port, "(port) \(message())", file: file, line: line)
        XCTAssertEqual(
            uri.path, path.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed), "(path) \(message())", file: file, line: line)
        XCTAssertEqual(
            uri.query, query?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed), "(query) \(message())", file: file, line: line
        )
        XCTAssertEqual(
            uri.fragment, fragment?.addingPercentEncoding(withAllowedCharacters: .urlFragmentAllowed), "(fragment) \(message())",
            file: file, line: line)
...
final class URITests: XCTestCase {
    ...
    func testUrlParsingVectors() {
        ...
        XCTAssertURIString(
            "http://host.com/foo/bar/../index.html", hasScheme: "http", hasHost: "host.com", hasPath: "/foo/bar/../index.html")
