...
class LeafDataEncoderTests: XCTestCase {
    func testString() {
        let data = "hello"
        try XCTAssertEqual(LeafDataEncoder().encode(data), .string(data))
    }

    func testDouble() {
        let data: Double = 3.14
        try XCTAssertEqual(LeafDataEncoder().encode(data), .double(data))
    }

    func testDictionary() {
        let data: [String: String] = ["string": "hello", "foo": "3.14"]
        try XCTAssertEqual(LeafDataEncoder().encode(data), .dictionary([
            "string": .string("hello"),
            "foo": .string("3.14")
        ]))
    }
    ...
    func testArray() {
        ...
        try XCTAssertEqual(LeafDataEncoder().encode(data), .array([
