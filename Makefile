build: build_vapor
    cd .build; \
    swiftc ../Sources/*.swift -I . -L . -lVapor -lJay -lHummingbird -llibc -lStrand -o VaporApp

build_vapor: build_hummingbird build_jay build_vapor_libc
    cd .build; \
    swiftc ../Packages/Vapor/Sources/Vapor/**/*.swift -emit-library -emit-module -module-name Vapor -I . -L .

build_vapor_libc: fetch
    cd .build; \
    swiftc ../Packages/Vapor/Sources/libc/*.swift -emit-library -emit-module -module-name libc -I . -L .
...
fetch: clean prepare
    ...
    git clone https://github.com/qutheory/vapor Packages/Vapor; \
    cd Packages/Vapor; \
    git checkout 2-2-fixes
