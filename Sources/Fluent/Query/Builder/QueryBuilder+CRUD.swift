        ...
        return connection.flatMap(to: Void.self) { conn in
            ...
            return try model.willCreate(on: conn)
                .flatMap(to: Void.self) {
                    return self.run().map(to: Void.self) {
                        try model.parseID(from: conn)
                    }
                }
                .flatMap(to: Void.self) { try model.didCreate(on: conn) }
        }
    }
    ...
    public func update(_ model: Model) -> Signal {
        return connection.flatMap(to: Void.self) { conn in
            ...
            return try model.willUpdate(on: conn)
                .flatMap(to: Void.self) { self.run() }
                .flatMap(to: Void.self) { try model.didUpdate(on: conn) }
        }
    }
    ...
    internal func _delete(_ model: Model) -> Signal {
        return connection.flatMap(to: Void.self) { conn in
            return try model.willDelete(on: conn).flatMap(to: Void.self) {
                ...
                return self.run().flatMap(to: Void.self) { try model.didDelete(on: conn) }
            }
        }
    }
}
