    ...
    static func addTypePermutations(toArray paramsArray: [[Param]]) -> [[Param]] {
        ...
        for paramArray in paramsArray {
            for type in Param.types {
                ...
                var genericMap = ["T", "U", "V", "W", "X", "Y", "Z"]
                let generic = genericMap[count]
...
var functions = "extension Application {\n\n"

for method: Func.Method in [.Get, .Post] {
...
do{
    // writing to disk
    try functions.writeToURL(url, atomically: true, encoding: NSUTF8StringEncoding)
