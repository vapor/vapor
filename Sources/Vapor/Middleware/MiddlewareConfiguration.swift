...
public struct Middlewares: Sendable {
    ...
    private var storage: [AsyncMiddleware]
    ...
    public mutating func use(_ middleware: Middleware, at position: Position = .end) {
        ...
        if let providedMiddleware = middleware as? AsyncMiddleware {
            asyncMiddleware = providedMiddleware
        } else {
            asyncMiddleware = AsyncMiddlewareWrapper(middleware: middleware)
        }
        switch position {
        case .end:
            self.storage.append(asyncMiddleware)
        case .beginning:
            self.storage.insert(asyncMiddleware, at: 0)
        }
    }
    ...
    /// Resolves the configured middleware for a given container as `AsyncMiddleware`
    public func asyncResolve() -> [AsyncMiddleware] {
        return self.storage
    }
}
