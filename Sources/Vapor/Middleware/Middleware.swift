...
public protocol Middleware {
    ...
    func respond(to request: Request, on route: Route, chainingTo next: Responder) -> EventLoopFuture<Response>
}
...
private struct HTTPMiddlewareResponder: Responder {
    ...
    func respond(to request: Request, on route: Route) -> EventLoopFuture<Response> {
        return self.middleware.respond(to: request, on: route, chainingTo: self.responder)
    }
}
