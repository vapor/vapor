...
public struct URLEncodedFormCodingConfiguration {
    ...
    /// - parameters:
    ///     - bracketsAsArray: Set to `true` allows you to parse `arr[]=v1&arr[]=v2` as an
    ///                        array with key `arr`. Otherwise you would use  `arr` without  a
    ///                        bracket like `arr=v1&arr=v2`
    ///     - flagsAsBool: Set to `true` allows you to parse `flag1&flag2` as boolean variables
    ...
    ///     - arraySeparator: Uses this character to create arrays. If set to `,`, `arr=v1,v2` would
    ...
    public init(bracketsAsArray: Bool, flagsAsBool: Bool, arraySeparator: Character?) {
        // We don't provide default values as the defaults for encoder differ from the
        // decoder flagsAsBool is enabled for decoder, but not encoder.
