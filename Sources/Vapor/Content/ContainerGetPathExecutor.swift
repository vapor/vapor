    ...
    init(from decoder: Decoder) throws {
        guard let keypath = decoder.userInfo[.containerGetKeypath] as? [CodingKey] else {
            throw DecodingError.dataCorrupted(
                .init(codingPath: decoder.codingPath, debugDescription: "Container getter couldn't find keypath to fetch (broken Decoder?)")
            )
        }
...
extension CodingUserInfoKey {
    fileprivate static var containerGetKeypath: Self { .init(rawValue: "codes.vapor.containers.keypathget")! }
}
...
extension UnkeyedDecodingContainer {
    ...
    fileprivate func _unsafe_inplace_superDecoder() throws -> Decoder {
        var inplace = self
...
extension Decoder {
    ...
    fileprivate func unkeyedContainer(startingAt index: Int) throws -> UnkeyedDecodingContainer {
        ...
        guard container.currentIndex == index, !container.isAtEnd else {
            throw DecodingError.keyNotFound(
                BasicCodingKey.index(index), .init(codingPath: container.codingPath, debugDescription: "Missing index \(index)"))
        }
        return container
    }
}
