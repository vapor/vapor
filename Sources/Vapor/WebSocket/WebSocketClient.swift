...
//    public func webSocket(_ url: URLRepresentable, headers: HTTPHeaders = [:], beforeSend: (HTTPRequestContext) throws -> () = { _ in }) -> Future<WebSocket> {
//        do {
//            let req = HTTPRequestContext(http: .init(method: .GET, url: url, headers: headers), using: container)
//            try beforeSend(req)
...
//    /// - returns: A `Future` containing the newly connected `WebSocket` or an `Error`.
//    func webSocketConnect(_ request: HTTPRequestContext) -> Future<WebSocket>
//}
