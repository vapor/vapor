...
public struct BaseNEncoding: Sendable {
    ...
    internal static func sizeEnc(for bits: Int, count: Int) -> Int {
        ...
        let inputs = bits >> bits.trailingZeroBitCount  // number of output values per input bytes
    ...
    internal static func encode<C>(
        ...
        let mask = (1 << bits) - 1
        let outlen = self.sizeEnc(for: bits, count: decoded.count)
        let padding = self.padding(for: bits, count: outlen)
        ...
        return .init(unsafeUninitializedCapacity: outlen + padding) { p, n in
            ...
            for next in decoded {
                ...
                while bufBits >= bits {
                    bufBits &-= bits
                    p[n] = table[(buf &>> bufBits) & mask]
                    n &+= 1
                }
            }
            if padding > 0 {
                assert((1..<bits).contains(bufBits))
                p[n] = table[(buf &<< (bits &- bufBits)) & mask]
                n &+= 1
                if let pad = pad {
        ...
        return try? [UInt8].init(unsafeUninitializedCapacity: outlen) { p, n in  // N.B.: throwing is the only correct way to signal failure
            ...
            for next in encoded {
                ...
                case let moreBits:
                    ...
                    while bufBits >= 8 {
                        bufBits &-= 8
                        (p[n], n) = (UInt8(truncatingIfNeeded: buf &>> bufBits), n &+ 1)
                    }
                }
            }
    ...
    internal init(bits: Int, pad: UInt8? = nil, lookupTable: [UInt8], reverseTable: [UInt8: UInt8]? = nil, ignores: Set<UInt8> = []) {
        ...
        let realReverseTable = (UInt8.min ... .max).map { v -> UInt8 in
            if let b = reverseTable[v] {
                return b
            } else if ignores.contains(v) {
                return BaseNEncoding.ignoredByte
            } else if pad == v {
                return BaseNEncoding.paddingByte
            } else {
                return BaseNEncoding.invalidByte
            }
        }
    ...
    internal init(
        bits: Int, pad: Character? = nil, lookupTable: [Character], reverseTable: [Character: UInt8]? = nil, ignores: Set<Character> = []
    ) {
