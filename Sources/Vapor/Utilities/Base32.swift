    ...
    internal static func encode32(_ decoded: Span<UInt8>, pad: UInt8?, using tab: [UInt8]) -> [UInt8] {
        ...
        return .init(capacity: outlen + padding) { span in
            ...
            while inp.count &- idx >= 5 {
                ...
                span.append(tab[((b0 & 0xf8) &>> 3)])
                span.append(tab[((b0 & 0x07) &<< 2) | (b1 &>> 6)])
                span.append(tab[((b1 & 0x3e) &>> 1)])
                span.append(tab[((b1 & 0x01) &<< 4) | (b2 &>> 4)])
                span.append(tab[((b2 & 0x0f) &<< 1) | (b3 &>> 7)])
                span.append(tab[((b3 & 0x7c) &>> 2)])
                span.append(tab[((b3 & 0x03) &<< 3) | (b4 &>> 5)])
                span.append(tab[((b4 & 0x1f))])
                idx &+= 5
            }
            ...
            let b0 = rem > 0 ? get(0) : 0
            let b1 = rem > 1 ? get(1) : 0
            let b2 = rem > 2 ? get(2) : 0
            let b3 = rem > 3 ? get(3) : 0
            switch rem {
            case 0: return
            case 1:
                span.append(tab[((b0 & 0xf8) &>> 3)])
                span.append(tab[((b0 & 0x07) &<< 2)])
            case 2:
                span.append(tab[((b0 & 0xf8) &>> 3)])
                span.append(tab[((b0 & 0x07) &<< 2) | (b1 &>> 6)])
                span.append(tab[((b1 & 0x3e) &>> 1)])
                span.append(tab[((b1 & 0x01) &<< 4)])
            case 3:
                span.append(tab[((b0 & 0xf8) &>> 3)])
                span.append(tab[((b0 & 0x07) &<< 2) | (b1 &>> 6)])
                span.append(tab[((b1 & 0x3e) &>> 1)])
